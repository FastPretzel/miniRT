...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/10/06 16:29:34 by cgoth            #+#    #+#             */
/*   Updated: 2022/10/06 16:29:35 by cgoth           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	rotate(t_minirt *rt)
{
	if (rt->keys->key_w || rt->keys->key_s)
		rotate_xyz(rt, &(rt->camera->right), \
				&(rt->camera->up), &(rt->camera->dir));
	if (rt->keys->key_a || rt->keys->key_d)
		rotate_xyz(rt, &(rt->camera->up), \
				&(rt->camera->right), &(rt->camera->dir));
	if (rt->keys->key_q || rt->keys->key_e)
		rotate_xyz(rt, &(rt->camera->dir), \
				&(rt->camera->up), &(rt->camera->right));
}
